...
async def verify_attendance_image(
    ...
    try:
        ...
        error_msg = str(e)
        print(f"[ai_judge] Gemini error: {error_msg}")
        
        # DEMO MODE: If quota exceeded and demo mode enabled, auto-pass
        if DEMO_MODE and ("429" in error_msg or "quota" in error_msg.lower()):
            print("[ai_judge] ⚠️ DEMO MODE: Auto-passing verification due to API quota limit")
            return {
                "confidence": 95,
